...
#' Combine two chromosomal segments into one if segments are of same CN status and break smaller than defined threshhold
#'
...
combineSeg <- function(seq.dat, max.brk.len = 3e06, ap.calc = FALSE) {
  ...
  # For every segment expect for last
  for(i in 1:(n.segs - 1)) {
